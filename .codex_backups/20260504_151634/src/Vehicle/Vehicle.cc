...
QString Vehicle::payloadWeightText() const
{
    if (!_payloadWeightAvailable) {
        return QStringLiteral("--");

    }

    return QStringLiteral("%1 kg").arg(QString::number(_payloadWeightKg, 'f', 2));
}

void Vehicle::_updatePayloadWeightFromStatusText(const QString& messageText)
{
    static const QRegularExpression weightRegex(
        QStringLiteral("\\bWEIGHT\\s*:\\s*([-+]?\\d+(?:\\.\\d+)?)\\s*(kg)?\\b"),
        QRegularExpression::CaseInsensitiveOption);

    const QRegularExpressionMatch match = weightRegex.match(messageText);
    if (!match.hasMatch()) {
        return;
    }

    bool ok = false;
    const double parsedWeightKg = match.captured(1).toDouble(&ok);
    if (!ok) {
        return;
    }

    if (_payloadWeightAvailable && qFuzzyCompare(_payloadWeightKg + 1.0, parsedWeightKg + 1.0)) {
        return;
    }

    _payloadWeightKg = parsedWeightKg;
    _payloadWeightAvailable = true;
...
void Vehicle::_chunkedStatusTextCompleted(uint8_t compId)
{
    ...
    _updatePayloadWeightFromStatusText(messageText);
